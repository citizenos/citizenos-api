    ...
    suite('Activities', function () {
        ...
        suite('Read', function () {
            ...
            suiteSetup(async function () {
                ...
                await topicLib.topicCreate(agent, user.id, null, Topic.STATUSES.inProgress);
            });
...
suite('Activities', function () {
    ...
    suite('Read', function () {
        ...
        suiteSetup(async function () {
            ...
            topic = (await topicLib.topicCreate(agent, user.id, 'TEST', Topic.STATUSES.inProgress, '<html><head></head><body><h2>TEST</h2></body></html>', Topic.VISIBILITY.public)).body.data;
    ...
    suite('Count', function () {
        ...
        test('Success', async function () {
            const activities = (await activitiesRead(agent, user.id, null)).body.data
            assert.isTrue(activities.length > 0);
            await topicLib.topicCreate(agent, user.id, 'TEST3', Topic.STATUSES.inProgress, '<html><head></head><body><h2>TEST3</h2></body></html>', Topic.VISIBILITY.public);
