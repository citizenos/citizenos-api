...
module.exports = function (app) {
    ...
    const search = async (query, user) => {
        const str = query.str; // Search string
        const limitMax = 100;
        ...
        const fields = [];
        let limit = parseInt(query.limit, 10) ? parseInt(query.limit, 10) : limitDefault;
        if (limit > limitMax) limit = limitDefault;
        const page = parseInt(query.page, 10) ? parseInt(query.page, 10) : 1;
        const offset = query.offset || (page * limit) - limit;
        const params = Object.keys(query);
        let statuses;
        let queryStatuses = query.statuses;
                    ...
                    if (model === 'topic') {
                        ...
                        fields.forEach(function (field) {
                            ...
                            if (fieldBuild[0] === 'my' && fieldBuild[1] === 'topic') {
                                if (fieldBuild[2] === 'level' && query[field] !== 'none') {
                                    levelComparer = '>=';
                                    level = query[field];
                                }
                            }
